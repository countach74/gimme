...
def json():
  class wrapper(decorator):
    def __call__(self, *args, **kwargs):
      raw_output = self.fn(*args, **kwargs)
      self.get_fn().instance.response.set('Content-Type', 'application/json')
  ...
  def make_wrapper(content_type):
    class wrapper(decorator):
      ...
      def __call__(self, *args, **kwargs):
        ...
        priority_accept = accepted.get_highest_priority_for_mime(formats.keys())
        if priority_accept:
          method = formats[priority_accept]
          self.get_fn().instance.response.set('Content-Type', priority_accept)
